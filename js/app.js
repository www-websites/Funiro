(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".slider-rooms__slider")) new core(".slider-rooms__slider", {
            modules: [ Navigation, Pagination, Parallax ],
            observer: true,
            observeParents: true,
            slidesPerView: "auto",
            ...
            loop: true,
            pagination: {
                el: ".slider-rooms-pagination",
                clickable: true
            },
            navigation: {
                prevEl: ".slider-rooms-button_prev",
                nextEl: ".slider-rooms-button_next"
            },
            on: {}
        });
        if (document.querySelector(".tips__slider")) new core(".tips__slider", {
            ...
            navigation: {
                prevEl: ".tips-button_prev",
                nextEl: ".tips-button_next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1,
...
***************************************************************************** */
    ...
    function documentActions(e) {
        ...
        if (isMobile.any()) if (document.documentElement.clientWidth > 767) if (targetElement.closest(".item-menu__arrow")) targetElement.closest(".item-menu__arrow").closest(".item-menu").classList.toggle("_active"); else if (!targetElement.closest(".item-menu__arrow")) {
            const menuItems = document.querySelectorAll(".item-menu");
            for (const iterator of menuItems) iterator.classList.remove("_active");
        }
        if (targetElement.classList.contains("popup-products__button")) {
            const productId = targetElement.closest(".item-products").dataset.pid;
            addToCart(targetElement, productId);
            e.preventDefault();
        }
        if (targetElement.classList.contains("cart__delete")) {
            const productId = document.querySelector(".cart__item").dataset.cartPid;
            updateCart(targetElement, productId, false);
            e.preventDefault();
        }
    }
    const headerSearchButton = document.querySelector(".header__search-button");
    const headerSearchWrapper = document.querySelector(".header__form");
    headerSearchButton.addEventListener("click", (function(e) {
        headerSearchWrapper.classList.toggle("_active");
    }));
    function addToCart(productButton, productId) {
        if (!productButton.classList.contains("_hold")) {
            productButton.classList.add("_hold");
            productButton.classList.add("_fly");
            const cart = document.querySelector(".actions-header_cart");
    ...
    function updateCart(productButton, productId, productAdd = true) {
        ...
        if (productAdd) {
            if (cartQuantity) cartQuantity.innerHTML = ++cartQuantity.innerHTML; else cart.insertAdjacentHTML("beforeend", `<span>1</span>`);
            if (!cartProduct) {
                const product = document.querySelector(`[data-pid="${productId}"]`);
                const cartProductImage = product.querySelector(".item-products__image-ibg").innerHTML;
                const cartProductTitle = product.querySelector(".item-products__title").innerHTML;
                const cartProductText = product.querySelector(".item-products__description").innerHTML;
                const cartProductPrice = product.querySelector(".item-products__price").innerHTML;
                const cartProductContent = `\n\t\t\t<div class="cart__image-ibg">\n\t\t\t\t${cartProductImage}\n\t\t\t</div>\n\t\t\t<div class="cart__body">\n\t\t\t\t<div class="cart__title">${cartProductTitle}</div>\n\t\t\t\t<div class="cart__description">${cartProductText}</div>\n\t\t\t\t<div class="cart__price">${cartProductPrice}</div>\n\t\t\t\t<div class="cart__quantity">Quantity : <span>1</span></div>\n\t\t\t\t<div class="cart__actions">\n\t\t\t\t\t<button type="button" class="cart__delete">Remove</button>\n\t\t\t\t\t<button type="button" class="cart__buy">Buy</button>\n\t\t\t\t</div>\n\t\t\t</div>`;
                cartItems.insertAdjacentHTML("beforeend", `<li data-cart-pid="${productId}" class="cart__item">${cartProductContent}</li>`);
            } else {
                const cartProductQuantity = cartProduct.querySelector(".cart__quantity span");
